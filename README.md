...
- **Cartões de produto**: cada um mostra uma foto real do equipamento (em um cartão branco arredondado) sobre um fundo em gradiente azul, nome, especificações técnicas, características extras, preço em R$ e status de estoque (Em estoque / Estoque baixo). Ao passar o mouse sobre a foto, ela gira em 3D (CSS `rotateY`) e mostra o produto espelhado, como se estivesse girando para o outro lado. As fotos estão em `assets/products/` e foram obtidas de fotos de produto reais para fins de demonstração/apresentação — este é um projeto de protótipo, não uma loja em produção.
